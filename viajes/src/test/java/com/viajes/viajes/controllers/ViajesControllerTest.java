...
import static com.mongodb.ReadConcern.AVAILABLE;
import static com.viajes.viajes.enumns.TripStatus.*;
...
public class ViajesControllerTest {
 ...
 void setUp() {
    ...
    public void testGetViajesPorScooterID(){
        ...
        assertEquals(HttpStatus.OK, response.getStatusCode());
        Mockito.verify(viajesService, times(1)).getAllByScooterID(mockscooter.getId());
    }
    ...
    public void testPausarViaje(){
        ...
        mockedtrip.setTripStatus(PAUSED);
        ...
        assertEquals(mockedtrip.getTripStatus() , PAUSED);
        Mockito.verify(viajesService, Mockito.times(1)).setPauseTrip(mockedtrip.getTripID());
    }
    @Test
    public void testReanudarViaje(){
        ...
        TripOutputDTO mockedtrip = new TripOutputDTO();
        mockedtrip.setTripID("tripID");
        mockedtrip.setTripStatus(STARTED);
        mockedtrip.setEndPauseTime(LocalDateTime.now());

        Mockito.when(viajesService.setUnPauseTrip(mockedtrip.getTripID())).thenReturn(mockedtrip);

        assertEquals(mockedtrip.getTripStatus() , STARTED);
        Mockito.verify(viajesService, Mockito.times(1)).setPauseTrip(mockedtrip.getTripID());
    }
